...
exports.run = async (client, message, args) => {
  ...
  var Vokal = message.guild.roles.get("781958052269129760")
  
if(!user) return message.reply ("Lütfen bir kullanıcı etiketleyiniz").then(m => m.delete(5000));

  
        if(!Vokal) return message.channel.send ("Vokal Rolü Yok").then(m => m.delete(5000));


  if(!user.roles.has(Vokal.id)){
  
    await (user.addRole(Vokal.id))
    
  
   message.react('✅')
    let embed = new Discord.RichEmbed()
    .setColor(Vokal.color)
    .setDescription(`${user.user} Kullanıcısına <@&${Vokal.id}> Rolü Verildi.`)
  ...
  .setTimestamp()  
    ...
    await (user.removeRole(Vokal.id));
    ...
    .setColor(Vokal.color)
    .setDescription(`${user.user} Kullanıcısına <@&${Vokal.id}> Rolü Alındı.`)
   .setFooter(`${message.author.tag}` , `${message.author.displayAvatarURL}`)
  .setTimestamp()  
